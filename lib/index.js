...
// src/license/getLicense.ts
var NPM_BASE_URL = "https://registry.npmjs.com";
var FALLBACK_LICENSE_VALUE = "";
var getLicense = async (name, version) => {
  try {
    const url2 = `${NPM_BASE_URL}/${name}/${version}`;
...
// src/getExternalDependencies.ts
var getExternalDependencies = (packageJson, internalDepsRegex = /^@surikat/) => Object.entries(packageJson.dependencies).concat(Object.entries(packageJson.devDependencies)).filter(([name]) => !internalDepsRegex.test(name)).reduce(
  (acc, [name, version]) => acc.concat({
    name,
    version,
...
// src/getPackageJson.ts
var import_promises = __toESM(require("fs/promises"));
var import_path = __toESM(require("path"));
var getPackageJson = async () => {
  const resolvedPath = import_path.default.resolve("./package.json");
  const pkgJson = await import_promises.default.readFile(resolvedPath, { encoding: "utf-8" });
  return JSON.parse(pkgJson);
};

// src/license/updateLicenses.ts
var core = __toESM(require_core());

// src/checkSafeInt.ts
var checkSafeInt = (text) => text !== "" && Number.isSafeInteger(Number(text));

// src/version/getMinorVersion.ts
var getMinorVersion = (version) => {
  try {
...
// src/version/getExactVersion.ts
var getExactVersion = (version) => {
  try {
...
// src/version/getPatchVersion.ts
var getPatchVersion = (version) => {
  try {
...
// src/version/getNpmVersion.ts
var getNpmVersion = (version) => getExactVersion(version) || getPatchVersion(version) || getMinorVersion(version) || version || "";

// src/license/updateLicenses.ts
var updateLicenses = async (dependencies) => {
  await Promise.all(
...
async function run() {
  try {
    ...
    const dependencyPattern = core2.getInput("internal-dependency-pattern");
    const pattern = dependencyPattern ? new RegExp(dependencyPattern) : void 0;
    const dependencies = getExternalDependencies(packageJson, pattern);
    await updateLicenses(dependencies);
    core2.setOutput("external-dependencies", dependencies);
    core2.debug(JSON.stringify(dependencies, null, 2));
  } catch (error) {
